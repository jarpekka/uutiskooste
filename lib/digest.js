...
function limitItemsPerSource(items, maxItems, perSourceLimit = MAX_ITEMS_PER_SOURCE) {
...
async function checkSourceHealth(options, customSources = []) {
  ...
  const results = fetched.map((entry) => {
    const source = allSources.find((candidate) => candidate.name === entry.source);
    const acceptableItems = entry.results.filter((item) => source && isAcceptable(item, source, options));
...
async function buildDigest(options, customSources = []) {
  const custom = prepareCustomSources(customSources);
  const allSources = [...SOURCES, ...custom.sources];
  ...
  const sections = AREAS.filter((area) => options.areas.includes(area.id)).map((area) => {
    ...
    const limitedItems = limitItemsPerSource(deduped.items, options.maxPerArea);
